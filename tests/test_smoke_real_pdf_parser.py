...
class RealPdfParserSmokeTest(unittest.TestCase):
    """真实 PDF 解析链路冒烟测试。"""

    @unittest.skipUnless(MARKITDOWN_AVAILABLE, "markitdown 未安装")
    def test_real_pdf_parser_pipeline_smoke(self) -> None:
        """real 模式下应使用真实 PDF 解析器跑通主流程。"""
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            config = RuntimeConfig(
                ...
                components={"generator": {"name": "mock"}},
            )
            ...
            self.assertEqual(payload["entries"][0]["adapter"], "pdf_cpu")
            self.assertTrue(payload["entries"][0]["exists"])
            self.assertEqual(len(payload["entries"][0]["parsed_doc_id"]), 32)
            self.assertIn("/parser/", payload["entries"][0]["artifact_path"])
            self.assertEqual(len(payload["doc_id_mapping"]), 1)
            self.assertEqual(
                payload["doc_id_mapping"][0]["parsed_doc_id"],
                payload["entries"][0]["parsed_doc_id"],
            )
