...
class QAGeneratorBase(LoopBatchMixin[List[DocumentChunk], List[QAPair]], ABC, Generic[TGeneratorConfig]):
    """QA 生成器接口。"""

    config: TGeneratorConfig
    single_method_name: str = "generate"

    def __init__(self, config: Optional[TGeneratorConfig] = None) -> None:
    ...
    def generate_batch(
        self, batch_chunks: Iterable[List[DocumentChunk]], ctx: RunContext
    ) -> List[List[QAPair]]:
